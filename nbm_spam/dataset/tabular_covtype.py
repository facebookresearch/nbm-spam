...
class CovType(Dataset):
    def __init__(
        ...
        self.num_classes = max(_data.target)
        ...
        if self.split == "train":
            ...
            self.labels = (
                torch.Tensor(_l_train).to(device=self.device, dtype=torch.int64) - 1
            )
        elif self.split == "validation":
            self.features = torch.Tensor(_f_val).to(device=self.device)
            self.labels = (
                torch.Tensor(_l_val).to(device=self.device, dtype=torch.int64) - 1
            )
        elif self.split == "test":
            self.features = torch.Tensor(_f_test).to(device=self.device)
            self.labels = (
                torch.Tensor(_l_test).to(device=self.device, dtype=torch.int64) - 1
            )
